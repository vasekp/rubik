...
void main() {
  vec4 proper_colour = texture(sampler, texCoord);
  vec4 nnormal = normalize(normal);
  vec4 npos = vec4(normalize(vec3(coords)), 1);
  vec3 diffuse = (max(dot(nnormal, light_dir), 0) + ambient_int) * vec3(proper_colour);
  ...
  colour = vec4(mix(diffuse, specular, mix_specular), 1) * faceColour;
}
