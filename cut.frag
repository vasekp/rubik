...
void main() {
  vec4 properColour = texture(sampler, texCoord).r * faceColour;
  properColour = mix(properColour, vec4(vec3(base_brightness), 1), 1 - properColour.a);
  vec4 nnormal = normalize(normal);
  vec4 npos = vec4(normalize(vec3(coords)), 1);
  float diffuse_int = (max(dot(nnormal, light_dir), 0) + ambient_int);
